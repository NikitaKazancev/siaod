...
void showCommands() {
	...
	<< "5 - check expression\n"
	<< "6 - show expression\n"
	<< "7 - does expression contain multiply\n"
	<< "0 - show commands\n";
}
...
int main() {
	...
	while (true) {
		...
		if (command == "7") {
			cout << (contains(tree->expression, '*') ? 
			"\nExpression contains operation \"multiply\" (*)\n" : 
			"\nExpression does not contains operation \"multiply\" (*)\n");
		} else {
			showCommands();
		}
	}
